...
class BaseGPT:
    ...
    def _build_params(self, messages, response_format, functions, function_call, parallel_tool_calls, is_main_call):
        params = {
    ...
    async def _stream_response(self, params):
        ...
        async for chunk in response:
            ...
            if chunk.choices and chunk.choices[0].delta.content is not None:
                ...
                yield (custom_return, self._format_response(self.buffer, current_chunk, False))

            if chunk.choices and chunk.choices[0].delta.tool_calls is not None:
                tool_calls = self._append_tool_calls(tool_calls, chunk.choices[0].delta.tool_calls)
    ...
    async def _make_ai_call(
        ...
        while True:
            params = self._build_params(
                messages, response_format, functions, function_call, parallel_tool_calls, is_main_call
            )
            ...
            if function_call_detected:
                messages.append({"tool_calls": function_call_detected, "role": 'assistant',})

                for tool_call in function_call_detected:
                    function_result, function_metadata = await self._handle_function_call(
                        tool_call
                    )
                    ...
                    cc = json.dumps(
                        {"content": function_metadata, "replace": False, "type": "metadata"}
                    )
                    yield f"data: {cc}\n\n"
            else:
                break
